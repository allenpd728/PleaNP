...
| `docs/MATHLIB_SUBMISSION.md` | **Submitting to Mathlib (2026-09-06):** the real process (Zulip-first, small PRs, bors/CI), the 5 realistic rejection paths (too-early, too-big, design-objection, duplicate, substantive-criticism) and how to handle each, and the reputation non-negotiables (never submit a sorry; submit the oracle substrate not the barrier theorems; cite prior art; follow process; one human anchor). Read before any Mathlib submission. |
