...
| `formalization.yaml` | **Repo-root machine-readable statement manifest (v0.4, DEC-022):** per declaration: informal source ↔ Lean declaration ↔ file ↔ sorry_count ↔ axioms ↔ comparator_config ↔ review status. Declarative data;the gate scanners remain the executables. Add/update rows in the same commit as every proof freeze (same rule as `docs/SORRY_TRACKER.md`). |
