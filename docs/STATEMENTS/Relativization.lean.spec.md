# Spec: `lean/PleaNP/Barriers/Relativization.lean` (the BGS statement rendering)

**Rung:** 3a — the Lean rendering of the Baker–Gill–Solovay theorem statement into `lean/PleaNP/Barriers/Relativization.lean`. This is the Gate-1 frozen statement (the formal Lean anchor), rendered against `P^A`/`NP^A` from `lean/PleaNP/Computability/OracleComplexity.lean`.
...
Per `Relativization.md` §3 (the frozen informal statement), the BGS theorem has two existence clauses. Each becomes a `theorem` (or `example`/`def`-witness) in `lean/PleaNP/Barriers/Relativization.lean`:
...
1. **The oracle is existential and total.** The `A`/`B` is `∃ A : Oracle Q, ...` — the oracle type from `lean/PleaNP/Computability/Oracle.lean` (`Oracle Q := Q → Bool`), which is total by construction. The rendering must NOT quantify over partial oracles (e.g. `Part Bool`-valued) — that's a different (and in clause (a)'s case trivial) theorem. Gate 2 checks the oracle type is `PleaNP.Oracles.Oracle`.
...
3. **`P^A = NP^A` is set extensional equality.** The `=` must be set equality of two language classes (per `OracleComplexity.lean.spec.md` Trap 3). If `P^A`/`NP^A` are predicates (`Set α → Prop`), the rendering must use `∀ L, P^A L ↔ NP^A L` (predicate extensionality), not bare `=`. The choice follows whatever `lean/PleaNP/Computability/OracleComplexity.lean` decides (Trap 3 of that spec) — the rendering must be *consistent* with it.
...
If `P^A` and `NP^A` are `Set (Set α)`, then `P^A = NP^A` is set extensional equality (the spec's intended meaning). If they're `Set α → Prop` (predicates), then `P^A = NP^A` as written is *propositional equality* (which is stronger than extensional equivalence and may not be what's meant). The rendering must match the `lean/PleaNP/Computability/OracleComplexity.lean` choice (Trap 3 of that spec) and use the correct equality form.

**Acceptance criterion:** the rendering's `P^A = NP^A` (or `P^A L ↔ NP^A L`) matches the presentation in `lean/PleaNP/Computability/OracleComplexity.lean`. If `lean/PleaNP/Computability/OracleComplexity.lean` defines `P^A : Set (Set α)`, use `=`; if it defines `P^A : Set α → Prop`, use `∀ L, P^A L ↔ NP^A L`. Gate 4 (read-back) checks this: the read-back must say "the classes are equal as sets."
...
## 4. What "done" looks like for `lean/PleaNP/Barriers/Relativization.lean` (the statement)

Per `LOCAL_AGENT_WORKFLOW.md` Steps 1–5:
- **Substrate confirmed** — `lean/PleaNP/Computability/OracleComplexity.lean` is frozen (`P^A`/`NP^A` exist as real types).
...
- **Does not define `P^A`/`NP^A`.** Those are in `lean/PleaNP/Computability/OracleComplexity.lean`. This file *uses* them.
...
- **The substrate:** `lean/PleaNP/Computability/Oracle.lean` (v2, frozen) + the future `lean/PleaNP/Computability/OracleComplexity.lean`.
