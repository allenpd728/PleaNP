...
```
  (in `P_A`/`NP_A` this amounts to a function chosen per machine,and a proof obligation that the decode is *total* and *surjective-enough*:every oracle query a machine might need is expressible by some word — for BGS, `Q = Σ n, Bits n` and the word is the n-bit guess string itself (see §3.1). *Totality of the oracle itself is untouched* — the Oracle type is still total;decoding may be partial-per-encoding, but that lives in the machine-witness,not in the oracle. (The totality discipline of `Oracle.lean.spec.md` §3 applies to `Oracle Q`, not to `decode`.)
...
## 3. The BGS-facing acceptance details (why the fix keeps everything frozen
...
## 5. Acceptance criteria (all must hold before claiming #35 "done"
...
3. `OracleSmoke.lean` updated: the smoke machine now writes a query *word* onto tape kq (e.g. `1 0 1`), queries once, accepts/rejects per the two oracles — the executable check that the word-query path works end-to-end (and that the answer routes to yes/no labels correctly).
4. An explicit `example`/note pinning the 1-step property of the query transition (per §4.3(2)).
5. `P_A ⊆ NP_A` re-proved both directions; `P^∅ = P` compatibility re-stated (in `OracleUpstreamP.lean` if that module's statement referenced the v4 shape).
...
| #40 (Tests: v5 repair | #35 | test spec per §5(3)-(5) |