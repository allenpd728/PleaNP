# Spec: `lean/PleaNP/Computability/OracleComplexity.lean` (P^A / NP^A complexity classes)

**Rung:** 2 (local piece, the complexity layer on top of `lean/PleaNP/Computability/Oracle.lean`) — defines the oracle-relative complexity classes that the relativization barrier (`docs/STATEMENTS/Relativization.md`) quantifies over.
...
- The time bound applies to the *oracle machine's step count*, where each oracle query costs exactly 1 step (the `StepCount`/`EvalsToInTime` integration from `lean/PleaNP/Computability/Oracle.lean` Trap 2).
...
This carries the `P_empty_eq_upstream_P` statement from `lean/PleaNP/Computability/Oracle.lean` (Trap 3 of the recompose) *through* the complexity-class definitions. Once `P^A` is defined, the compatibility check becomes sharper: `P^∅` (P relative to the empty oracle) must equal upstream `P` (= `TM2ComputableInPolyTime` with no oracle queries). If `P^∅ ≠ P`, the class definition has quietly redefined P — the Gate 2 failure the whole architecture guards against.

**Acceptance criterion:** a `P_empty_eq_upstream_P`-style lemma (statement; proof may track upstream `P`), now stated at the *class* level: `P^(emptyOracle) = <upstream P>`. This is the model-consistency anchor for `lean/PleaNP/Computability/OracleComplexity.lean` specifically.
...
The encoding of nondeterminism (verifier vs. ND machine — §2.2) determines what `NP^A` *is*. A verifier-framing `NP^A` and an ND-machine-framing `NP^A` are (under standard assumptions) extensionally equal, but they're *different definitions*, and a `P^A = NP^A` proof that works under one encoding may not transfer to the other. The choice must be recorded and kept consistent across `lean/PleaNP/Computability/OracleComplexity.lean` and the eventual relativization proof.
...
Per §2.4: with the empty oracle, `P^∅` must equal upstream `P`. This is the check that defining `P^A` didn't quietly redefine P. It carries `lean/PleaNP/Computability/Oracle.lean`'s Trap 3 through the class layer.
...
## 4. What "done" looks like for `lean/PleaNP/Computability/OracleComplexity.lean`

Per `LOCAL_AGENT_WORKFLOW.md` status convention:
- **Substrate confirmed** — `lean/PleaNP/Computability/Oracle.lean` v2 is frozen (all recompose traps passed; `oracleQuery` real; `DecidesInTime` real; `StepCount` concrete).
...
- **Frozen** — the class definitions are canonical; the relativization statement (`∃ A, P^A = NP^A` / `∃ B, P^B ≠ NP^B`) can now be rendered in `lean/PleaNP/Barriers/Relativization.lean` against real `P^A`/`NP^A` types.
