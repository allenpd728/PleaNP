...
| **2 — Computational model / oracle machines** | Mathlib `TM2ComputableInTime` / `TM2OutputsInTime` / `initList` (core, v4.31.0); `Mathlib/Computability/RecursiveIn.lean` (upstream) (unbounded oracle computability) | **Imitate** the `TM2OutputsInTime` composition idiom for reachability; do **not** reuse `RecursiveIn` for time bounds (it has none). Track `UPSTREAM_TRACKING` for the P/NP model — import, never define. | 2026-08-19 |
...
| **3–8 — Statement-fidelity machinery (cross-cutting, DEC-022)** | `openai/NavierStokesAndEuler` (2026-09-08/10: Comparator challenge + `formalization.yaml` + standalone paper-theorem files, ~2,655 files/~640K lines, 0 sorries); `tristanbuckmaster/fluid_lean` (2026-09-08) | **Imitate** the Comparator pattern for every frozen barrier statement: independent externally-sourced challenge module, JSON pin (theorem names + `permitted_axioms`), `lake exe comparator` machine-check; proof root must **not** import the challenge. Also **imitate** the `formalization.yaml` (v0.4) manifest shape and the standalone-paper-theorem-file layout("freeze-the-claim-as-a-file"). Template: `docs/STATEMENTS/ComparatorChallenge.template.md`; background: `docs/LEAN_FORMALIZATION_LESSONS_2026-09-10.md`. Do **not** copy the "10,000-agent"/"publish-raw-agent-output" patterns (claim-sized agents;per-commit reviewável diffs). | 2026-09-10 |
...
- **Don't let a *choice* buy the conclusion ("chosen-to-work" anti-pattern, DEC-023,from the NS releases)** — a proof can compile, pass vacuity, and still be *creatively dishonest* if an exact choice(an after-the-fact oracle/force/encoding/or a fine-tuned parameter)does the work instead of an *internal mechanism*. Meet the standard "internal mechanism, externally-perturbable" (the NS calibration:the vortex/cascade phenomenon was robust even though the exact theorem needed fine tuning);audit which choices do the work and run the perturbation test(see `docs/VALIDATION_SUITE.md` §"Load-bearing-choice audit" and `docs/STATEMENTS/ProofIntuition.template.md` §3).A construction bought by an exact choice has thee same geometry as a post-hoc-selected Navier–Stokes force:it is non-vacuous-in-shape but not authoritative——flag it,don't silently accept it.
